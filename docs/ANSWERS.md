...
раздел 5 ниже), поэтому повторный отчёт по закрытому периоду навсегда вернёт те
...
Около четырёх часов за один заход с перерывами.

| Что | Часов |
|---|---|
| Разбор задания, перевод единицы выдачи с заказа на позицию | 0,75 |
| Возвраты за невыданное, сходимость на частичном заказе и после аварии | 0,5 |
| Недобросовестный поставщик: реестр кодов, эпохи запросов, карантин, нечестные режимы заглушки | 1 |
| Приоритеты в очереди и токен-бакет перед поставщиком | 0,5 |
| Append-only история и состояние на прошлый момент | 0,5 |
| Документация, ответы на вопросы, OpenAPI | 0,75 |

Больше всего ушло на вторую задачу, и снова не на код: реестр кодов и эпохи
запросов пишутся быстро, а времени стоило решить, что считать невалидным ответом
и почему на таймаут эпоха расти не должна. Дороже всего в правках оказалась
первая: смена единицы выдачи с заказа на позицию прошла через все слои и через
уже написанные тесты первого этапа.

Обе бонусные задачи обошлись дёшево, потому что первый этап уже держал нужную
форму. Очередь лежала в той же транзакции, что и изменение состояния, так что
приоритет добавился колонкой, а лимит поставщика одной таблицей. Журнал денег и
так был append-only по соглашению, и история легла поверх уже записанных фактов,
добавив к ним триггер вместо соглашения.

---

## 4. Готовность внести небольшое изменение на звонке

Готов. Стенд поднимается одной командой, полный прогон тестов занимает около двух
с половиной минут, так что изменение вносится и проверяется прямо в разговоре.

---

## 5. Дополнительно: восстановление картины на любой момент
...
## 6. Дополнительно: всплеск заказов и лимит поставщика
